...
export const Hero: FC<{ airports: string[] }> = ({ airports }) => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative z-10 flex flex-col items-start md:items-center justify-center gap-4 bg-accent px-4 pt-8 pb-16 lg:px-0 lg:pt-36"
    >
      <div className="-z-10 absolute inset-0 flex w-full items-center justify-center -mt-48">
        <motion.svg
          style={{ y: logoY }}
      ...
      <div className="h-48 hidden 2xl:block"></div>
      <motion.h2
        style={{ y: textY }}
        className="font-display text-6xl relative flex md:gap-4 flex-col md:flex-row"
      >
        <span className="font-bold">I am</span>{' '}
        <span className="font-black text-primary-1">Cameron Burrows</span>
      </motion.h2>
      <motion.h3 style={{ y: textY }} className="text-3xl text-tertiary-2">
        Developer <span className="text-tertiary-3">&amp;</span> Traveller
      </motion.h3>
      <div className="relative z-10 pt-8 w-full flex items-center justify-center">
        <BoardingPass airports={airports} />
      </div>
    </section>
  )
}
